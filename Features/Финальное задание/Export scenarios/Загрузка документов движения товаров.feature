...
* Перед проведением эталонных документов распроведем или удалим все остальные
	...
	Если я выбираю пункт контекстного меню с именем 'СписокКонтекстноеМенюОтменаПроведения' на элементе формы с именем "Список" Тогда
...
* Загрузка эталонных документов
	...
	И я перезаполняю для объекта табличную часть "Товары":
		...
		| 'e1cib/data/Документ.РасходТовара?ref=bbf30050ba5c887711e1fe6190994fe4' | 'e1cib/data/Справочник.Товары?ref=8d3a000d8843cd1b11dd321ba7a30aaf' | 250    | 1            | 250     |
