...
Сценарий: Подготовительный сценарий (движения документа Продажа товарав)
	Когда Загрузка основных данных
	Когда Загрузка документов движения товаров

Сценарий: Проверка движений документа Продажа товара по регистру Взаиморасчеты
...
Сценарий: Проверка движений документа Продажа товара по регистру Продажи
	Дано я закрываю все окна клиентского приложения
	И я открываю навигационную ссылку "e1cib/list/Документ.РасходТовара"
	И в таблице "Список" я перехожу к строке:
		| 'Номер'     |
...
Сценарий: Проверка движений документа Продажа товара по регистру Товарные запасы
	Дано я закрываю все окна клиентского приложения
	И я открываю навигационную ссылку "e1cib/list/Документ.РасходТовара"
	И в таблице "Список" я перехожу к строке:
		| 'Номер'     |
